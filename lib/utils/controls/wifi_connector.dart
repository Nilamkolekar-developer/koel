...
// Note: In Dart, we don't have Method Overloading.
// I've used optional parameters or renamed methods to handle C# overloads.

abstract class WifiConnector implements Disposable {
  ...
  Future<String> getDongleMacID(
      String ip, bool isLengthFFF, bool isChannels, String channelId);

  // Handled C# overloads using optional named parameters
  ...
  Future<List<ReadPidResponseModel>> setRoutineValue(
      List<PidCode> pidList, String pidByAddrSeq, Uint8List actualResponse);
  ...
  Future<String> clearDtc(
      String indexKey, String seedKeyIndex, String writePidIndex);
  ...
  // Renamed to handle overloading
  Future<List<ReadPidResponseModel>> readPidByAddr(
      List<PidCode> pidList, String pidByAddrSeq);

  Future<List<ReadPidResponseModel>> readPidParams(
      List<ReadParameterPid> pidList);

  Future<List<WriteParameterStatus>> writePid(String writePidIndex,
      List<WriteParameterPid> pidList, String pidByAddrSeq);

  Future<String> startECUFlashing(String flashJson, String interpreter,
      SeedkeyalgoFnIndex sklFIN, List<EcuMapFile> ecuMapFile);

  Future<WriteParameterStatus> writeAtuatorTest(String writeParaIndex,
      String seedKeyIndex, List<Uint8List> command, bool isStartTest);

  Future<TestRoutineResponseModel> setTestRoutineCommand(
      String seedKey, String writeParaIndex, String startCommand);
  ...
  Future<List<ReadPidResponseModel>> ivnReadPid(
      List<IvnSelectedPid> ivnPidList);
...
// Simple interface for IDisposable
