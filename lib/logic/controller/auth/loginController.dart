...
//   }
  ...
  // ---------------- STATE ----------------
  var userRequestModel = UserModel(username: '', password: '', macId: '', deviceType: '').obs;
  ...
  Future<void> initData() async {
    ...
    appVersion.value = "App Version 1.0.0"; // replace with package_info

    if (prefs.getString("is_remember") == "true") {
      userRequestModel.update((val) {
        val?.username = prefs.getString("user_email") ?? "";
  ...
  Future<void> login() async {
    final user = userRequestModel.value;

    if ((user.username ).isEmpty &&
        (user.password ?? "").isEmpty) {
      showAlert("Enter user name and password.");
      return;
    }
    if ((user.username ?? "").isEmpty) {
    ...
    if ((user.password ?? "").isEmpty) {
      showAlert("Enter user password.");
      return;
    }

    try {
      bool isOnline = true; // TODO: use connectivity_plus

      bool isLogging =
          isOnline ? await loginOnline() : await loginOffline();

      if (isLogging) {
        final prefs = await SharedPreferences.getInstance();

        if (rememberCheck.value) {
          prefs.setString("user_email", userResModel?.user ?? "");
          prefs.setString("user_password", user.password ?? "");
          prefs.setString("is_remember", "true");
        } else {
          prefs.setString("user_email", "");
          prefs.setString("user_password", "");
          prefs.setString("is_remember", "false");
        }

        App.userResModel = userResModel!;

        // Navigation (replace with your screen)
        Get.offAll(() => const ());
      }
    } catch (e) {
  ...
  Future<bool> loginOnline() async {
    try {
      final response = await AuthApiService.login(userRequestModel.value);
      userResModel = response;
      return true;
    } catch (e) {
      return false;
    }
  ...
  void togglePassword() {
    ...
    imgPassword.value =
        isPassword.value ? "ic_password.png" : "ic_visible.png";
  }
